document.addEventListener('DOMContentLoaded', function () {
    ...
    let currentClientTheme = localStorage.getItemdocument.addEventListener('DOMContentLoaded', function () {
    console.log("Silent Sense APP (Novo Design V4) DOM Carregado - Iniciando Script...");
        ...
        tableHtmlContent += `
            <tr data-aluno-id="${aluno.id}">
                ...
                <td class="actions-cell">
                    <button class="btn btn-small btn-link aluno-view-btn" title="Ver Perfil"><i href="indexAluno.html" class="fas fa-eye"></i></button>
                    <button class="btn btn-small btn-link aluno-edit-btn" title="Editar"><i href="indexAluno.html" class="fas fa-edit"></i></button>
...
appTheme') || 'light';
   ...
   async function toggleAndSaveAdminMode() { 
    ...
    // --- PARTE 1: Contagem de Alunos (sempre executa) ---
    const alunosCount = cachedAlunos ? cachedAlunos.length : 0;
    const alunosAutistasCount = cachedAlunos ? cachedAlunos.filter(a => a.autista).length : 0;
    if (dashTotalAlunosEl) dashTotalAlunosEl.textContent = alunosCount;
    if (dashAlunosAutistasEl) dashAlunosAutistasEl.textContent = alunosAutistasCount;

    // --- PARTE 2: Lógica dividida por Modo (Simulação vs. Real) ---
    if (cachedConfigs.simulacaoAtiva) {
        // --- MODO SIMULAÇÃO ATIVO ---
        const currentNoise = Math.floor(Math.random() * 70) + 30;
        if (dashNoiseLevelBarEl) dashNoiseLevelBarEl.style.width = `${Math.min(100, currentNoise / 95 * 100)}%`;
        if (dashNoiseValueEl) dashNoiseValueEl.textContent = `${currentNoise} dB`;
        
        // Lógica para o valor máximo em modo de simulação
        if (dashMaxNoiseEl) {
             const maxText = dashMaxNoiseEl.textContent || 'Máx: 0 dB';
             const maxAtual = parseInt(maxText.replace(/[^0-9]/g, ''), 10) || 0;
             if (currentNoise > maxAtual) {
        ...
        // --- MODO REAL (SENSOR DO ARDUINO) ---
        
        // 1. Lógica do Nível de Ruído (Nova e Melhorada)
        const valorDoSensor = ultimoValorSensorReal; // Pega da variável global
        let displayValor = "---";
        let noiseNumber = 0;

        // Verifica se o valor recebido é um número (como será enviado pelo novo servidor)
        if (typeof valorDoSensor === 'number') {
            noiseNumber = valorDoSensor;
            displayValor = `${noiseNumber} dB`;
        } else {
            // Se não for um número, é um status como "Conectando..."
            displayValor = valorDoSensor;
        }

        // Atualiza a UI do Ruído com os dados reais
        const barWidth = Math.min(100, (noiseNumber / 95) * 100); // Calcula a % baseada no seu máx de 95dB
        if (dashNoiseLevelBarEl) dashNoiseLevelBarEl.style.width = `${barWidth}%`;
        if (dashNoiseValueEl) dashNoiseValueEl.textContent = displayValor;
        ...
        // 2. Lógica de Incidentes e Sala (Dados Reais)
        const hojeStr = new Date().toISOString().split('T')[0];
        const incidentesDeHoje = cachedRelatorios ? cachedRelatorios.filter(r => r.tipo === 'incidente' && r.data?.startsWith(hojeStr)) : [];
        if (dashIncidentesHojeEl) dashIncidentesHojeEl.textContent = incidentesDeHoje.length;
    ...
    // --- PARTE 3: Renderiza os gráficos (sempre executa no final) ---
    renderDashboardChartsUI();
}
